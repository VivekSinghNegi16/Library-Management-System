class BookDAO():
	...
	def add(self, book):
		...
		if book['availability'] == 'off':
			available = 0
		else:
			available = 1

		'''When description works'''
		# q = self.db.query("INSERT INTO books (name, count, availability, edition, desc) VALUES('{}', '{}', '{}', '{}', '{}');".format(title, count, available, edition, description))
	...
	def edit(self, book):
			...
			# q = self.db.query(f"UPDATE @table set name={title}, count={count}, availability={available}, edition={edition}, desc={description} where id={book_id};")
			q = self.db.query(f"UPDATE @table set count={count}, availability={available}, edition={edition}, name='{title}', `desc`='{description}' where id={book_id};")
